...
beforeAll(async () => {
  const testDbUri =
    ...
    "mongodb://localhost:27017/nftopia-notifications-test";
...
describe("Notification Schema", () => {
  ...
  describe("Enum Validation", () => {
    ...
    it("should reject invalid status values", async () => {
      ...
      const notification = new Notification(notificationData);
      await expect(notification.save()).rejects.toThrow(
        "Status must be one of: pending, sent, failed, read"
      );
    });
  ...
  describe("Metadata Validation", () => {
    ...
    it("should require collection when nftId is provided", async () => {
      ...
      const notification = new Notification(notificationData);
      await expect(notification.save()).rejects.toThrow(
        "Collection name is required when NFT ID is provided"
      );
    });
  ...
  describe("Timestamps", () => {
    ...
    it("should update updatedAt when document is modified", async () => {
      ...
      expect(notification.updatedAt.getTime()).toBeGreaterThan(
