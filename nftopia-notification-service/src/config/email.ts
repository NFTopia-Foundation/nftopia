...
export const sendGridConfig = {
  ...
  templates: {
    nftPurchase: process.env.SENDGRID_TEMPLATE_PURCHASE! ?? "", 
    bidAlert: process.env.SENDGRID_TEMPLATE_BID! ?? "",
    passwordReset: process.env.SENDGRID_TEMPLATE_PASSWORD! ?? "",
    marketplaceAnnouncement: process.env.SENDGRID_TEMPLATE_ANNOUNCEMENT! ?? "",
    mint: process.env.SENDGRID_TEMPLATE_MINT! ?? "",
    nftSale: process.env.SENDGRID_TEMPLATE_SALE! ?? '',
    auctionUpdate: process.env.SENDGRID_TEMPLATE_AUCTION! ?? "",
    adminMessage: process.env.SENDGRID_TEMPLATE_ADMIN! ?? "",
  },
  sandboxMode: process.env.NODE_ENV! !== 'production'
};


export type EmailTemplateKey = keyof typeof sendGridConfig.templates;
