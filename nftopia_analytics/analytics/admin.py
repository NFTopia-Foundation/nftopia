...
class UserSessionAdmin(admin.ModelAdmin):
    list_display = [
        'user', 'login_at', 'logout_at', 'session_duration_display', 
        'ip_address', 'geographic_region', 'is_active'
    ]
...
class RetentionCohortAdmin(admin.ModelAdmin):
    list_display = [
        'cohort_date', 'period_type', 'period_number', 
        'total_users', 'retained_users', 'retention_rate_display'
    ]
    list_filter = ['period_type', 'cohort_date']
    ordering = ['-cohort_date', 'period_number']
    
    def retention_rate_display(self, obj):
        color = 'green' if obj.retention_rate >= 50 else 'orange' if obj.retention_rate >= 25 else 'red'
        return format_html(
            '<span style="color: {};">{:.2f}%</span>',
            color, obj.retention_rate
        )
...
class WalletConnectionAdmin(admin.ModelAdmin):
    list_display = [
        'user', 'wallet_provider', 'connection_status', 
        'attempted_at', 'wallet_address_short'
    ]
...
class UserBehaviorMetricsAdmin(admin.ModelAdmin):
    list_display = [
        'user', 'total_sessions', 'average_session_duration_display',
        'days_since_first_login', 'is_returning_user', 'preferred_wallet'
    ]
    list_filter = ['is_returning_user', 'preferred_wallet', 'first_login']
    search_fields = ['user__username', 'user__email']
    readonly_fields = [
        'first_login', 'last_login', 'total_sessions', 'total_session_time',
        'average_session_duration', 'days_since_first_login', 'last_updated'
    ]
...
class PageViewAdmin(admin.ModelAdmin):
    list_display = [
        'user_display', 'path', 'method', 'status_code', 
