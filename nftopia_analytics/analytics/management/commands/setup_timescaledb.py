    ...
    def create_extension(self):
        ...
        try:
            with connection.cursor() as cursor:
                ...
                self.stdout.write(
                    self.style.SUCCESS('✓ TimescaleDB extension enabled')
                )
            ...
            for hypertable in hypertables:
                try:
                    ...
                    if not cursor.fetchone()[0]:
                        self.stdout.write(
                            self.style.WARNING(f"⚠ Table {hypertable['table']} does not exist, skipping...")
                        )
                        continue
                    ...
                    if is_hypertable and not force:
                        self.stdout.write(
                            self.style.WARNING(f"⚠ {hypertable['table']} is already a hypertable")
                        )
                        continue
                    
                    if is_hypertable and force:
                        # Drop hypertable first
                        cursor.execute(f"SELECT drop_hypertable('{hypertable['table']}', true);")
                        self.stdout.write(f"✓ Dropped existing hypertable {hypertable['table']}")
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"✓ Created hypertable {hypertable['table']}")
                    )
                    
                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(f"✗ Failed to create hypertable {hypertable['table']}: {e}")
                    )
                    continue
            ...
            for table in tables_to_compress:
                try:
                    ...
                    if 'already exists' in str(e):
                        self.stdout.write(
                            self.style.WARNING(f"⚠ Compression policy already exists for {table}")
                        )
                    else:
                        self.stdout.write(
                            self.style.ERROR(f"✗ Failed to set compression for {table}: {e}")
                        ) 