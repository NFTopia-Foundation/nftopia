        ...
        with connection.cursor() as cursor:
            ...
            for table in raw_event_tables:
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"✓ Retention policy set for {table} ({raw_events_retention})")
                    )
                    
                except Exception as e:
                    if 'already exists' in str(e):
                        self.stdout.write(
                            self.style.WARNING(f"⚠ Retention policy already exists for {table}")
                        )
                    else:
                        self.stdout.write(
                            self.style.ERROR(f"✗ Failed to set retention policy for {table}: {e}")
                        )
                        continue
            ...
            for aggregate in aggregates:
                try:
                    if force:
                        ...
                        cursor.execute(f"DROP MATERIALIZED VIEW IF EXISTS {aggregate['name']} CASCADE;")
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"✓ Created continuous aggregate {aggregate['name']}")
                    )
                    
                except Exception as e:
                    if 'already exists' in str(e):
                        self.stdout.write(
                            self.style.WARNING(f"⚠ Continuous aggregate {aggregate['name']} already exists")
                        )
                    else:
                        self.stdout.write(
                            self.style.ERROR(f"✗ Failed to create aggregate {aggregate['name']}: {e}")
                        )
                        continue 