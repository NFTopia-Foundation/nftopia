...
def generate_mock_data():
    ...
    return {
        ...
        "volume": [abs(hash(str(d)+str(h)+str(c))) % 1000 
        ...
        "collection_id": [c 
                         for c in collections 
                         for _ in range(7*24)],
        "collection_name": [f"Collection {c}" 
                           for c in collections 
                           for _ in range(7*24)]
    }
...
def volume(request):
    try:
        ...
        if time_bin not in {'1h', '4h', '12h'}:
            return JsonResponse(
                {'error': 'Invalid time_bin. Use 1h, 4h or 12h'}, 
                status=400
            )
        collection_ids = request.GET.get('collection_ids', '')
        try:
            collections = [int(id) for id in collection_ids.split(',') if id] if collection_ids else None
        except ValueError:
            return JsonResponse(
                {'error': 'collection_ids must be comma-separated numbers'},
                status=400
            )
        ...
        if df.empty:
            return JsonResponse(
                {'error': 'No data for the parameters provided'},
                status=404
            )
