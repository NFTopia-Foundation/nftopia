...
pub trait IEscrow<TContractState> {
    ...
    fn get_swap(
        self: @TContractState, swap_id: u256,
    ) -> (ContractAddress, ContractAddress, u256, u256, u64, u8);
...
pub mod EscrowContract {
    ...
    #[abi(embed_v0)]
    pub impl EscrowImpl of super::IEscrow<ContractState> {
        fn create_swap(
            ...
            self
                .emit(
                    SwapCreated { swap_id, creator: caller, nft_contract, nft_id, price, expiry },
                );
        ...
        fn accept_swap(ref self: ContractState, swap_id: u256) {
            ...
            // Emit event
            self.emit(SwapAccepted { swap_id, acceptor: caller, accepted_at: current_time });
        ...
        fn cancel_swap(ref self: ContractState, swap_id: u256) {
            ...
            // Emit event
            self.emit(SwapCancelled { swap_id, cancelled_by: caller, cancelled_at: current_time });
        ...
        fn dispute_swap(ref self: ContractState, swap_id: u256) {
            ...
            // Emit event
            self.emit(SwapDisputed { swap_id, disputed_by: caller, disputed_at: current_time });

            self._unlock();
        }

        fn get_swap(
            self: @ContractState, swap_id: u256,
        ) -> (ContractAddress, ContractAddress, u256, u256, u64, u8) {
            let creator = self.swap_creators.read(swap_id);
            assert(creator.into() != 0, 'Swap does not exist');
    ...
    #[abi(embed_v0)]
    pub impl AdminImpl of super::IAdmin<ContractState> {
