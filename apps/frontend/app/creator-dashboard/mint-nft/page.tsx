...
import { useAuthStore } from '@/lib/stores/auth-store'; // Updated import

export default function MintNFTPage() {
  const router = useRouter();
  const { user, isAuthenticated } = useAuthStore(); // Using Zustand store
...
//           if (data.data.data.collections && data.data.data.collections.length > 0) {
//             setCollections(data.data.data.collections);
//             setSelectedCollectionId(data.collections[0].id); // Auto-select first collection
//           }
//         })
//         .catch(err => console.error('Error fetching collections:', err));
//     }
...
//   const handleSubmit = async (e: FormEvent) => {
//     e.preventDefault();
//     if (!user?.sub) return setError('User not authenticated');
//     if (files.length === 0) return setError('Please upload an image');
//     if (!selectedCollectionId) return setError('Please select a collection');
...
//       const res = await uploadToFirebase(files[0].file).then((firebaseUrl: string) => {
//         const mintData = {
//           imageUrl: firebaseUrl,
//           price,
//           currency,
...
//         <button
//           type="submit"
//           disabled={loading || !user || files.length === 0 || collections.length === 0}
//           className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-purple-600 to-blue-500 text-white py-2 px-4 rounded-lg hover:opacity-90 transition disabled:opacity-50"
//         >
//           <UploadCloud size={18} />
//           {loading ? 'Minting...' : 'Mint NFT'}
